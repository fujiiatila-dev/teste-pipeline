...
from connectors.asaas import AsaasConnector
...
def extract_asaas_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = AsaasConnector(
        api_url=credentials.get("api_url") or credentials.get("asaas_api_url"),
        access_token=credentials.get("access_token") or credentials.get("asaas_access_token"),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"asaas/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload Asaas MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
def asaas_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_ASAAS = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_ASAAS)

    asaas_clients = [c for c in clients if c.get("access_token") or c.get("asaas_access_token")]

    if not asaas_clients:
        print("Nenhum cliente Asaas encontrado na planilha.")
        return
    ...
    for client in asaas_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando Asaas: {company}")

        try:
            data = extract_asaas_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Asaas para {company}: {e}")
