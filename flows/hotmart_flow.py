...
from connectors.hotmart import HotmartConnector
...
def extract_hotmart_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = HotmartConnector(
        basic_auth=credentials.get("basic_auth") or credentials.get("hotmart_basic_auth"),
        days_to_fetch=int(credentials.get("days_to_fetch", 365)),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"hotmart/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload Hotmart MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="Hotmart to ClickHouse")
def hotmart_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_HOTMART = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_HOTMART)

    hotmart_clients = [
        c for c in clients
        if c.get("basic_auth") or c.get("hotmart_basic_auth")
    ]

    if not hotmart_clients:
        print("Nenhum cliente Hotmart encontrado na planilha.")
        return
    ...
    for client in hotmart_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando Hotmart: {company}")

        try:
            data = extract_hotmart_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Hotmart para {company}: {e}")
