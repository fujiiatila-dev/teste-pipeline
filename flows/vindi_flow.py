...
from connectors.vindi import VindiConnector
...
def extract_vindi_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = VindiConnector(
        api_token=credentials.get("api_token") or credentials.get("vindi_api_token"),
        api_url=credentials.get("api_url") or credentials.get("vindi_api_url"),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"vindi/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload Vindi MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="Vindi to ClickHouse")
def vindi_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_VINDI = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_VINDI)

    vindi_clients = [c for c in clients if c.get("vindi_api_token") or c.get("api_token")]

    if not vindi_clients:
        print("Nenhum cliente Vindi encontrado na planilha.")
        return
    ...
    for client in vindi_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando Vindi: {company}")

        try:
            data = extract_vindi_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Vindi para {company}: {e}")
