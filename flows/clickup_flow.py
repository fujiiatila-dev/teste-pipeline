...
def extract_clickup_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = ClickUpConnector(
        bearer_token=credentials.get("bearer_token") or credentials.get("clickup_bearer_token"),
        workspace_id=credentials.get("workspace_id") or credentials.get("clickup_workspace_id"),
        view_id=credentials.get("view_id") or credentials.get("clickup_view_id"),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"clickup/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload ClickUp MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="ClickUp to ClickHouse")
def clickup_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_CLICKUP = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_CLICKUP)

    clickup_clients = [
        c for c in clients
        if c.get("bearer_token") or c.get("clickup_bearer_token")
    ]

    if not clickup_clients:
        print("Nenhum cliente ClickUp encontrado na planilha.")
        return
    ...
    for client in clickup_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando ClickUp: {company}")

        try:
            data = extract_clickup_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar ClickUp para {company}: {e}")
