...
from connectors.imobzi import ImobziConnector
...
def extract_imobzi_data(date_start, date_stop, credentials):
    connector = ImobziConnector(
        api_secret=credentials.get("api_secret") or credentials.get("imobzi_api_secret"),
        api_url=credentials.get("api_url") or credentials.get("imobzi_api_url"),
    )
...
def load_to_clickhouse(data_dict, credentials, dt_stop):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"imobzi/{company_name}/{table_name}_run_{date_path}.parquet"
            print(f"[{table_name}] Upload Imobzi MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="Imobzi to ClickHouse")
def imobzi_pipeline(date_start=None, date_stop=None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_IMOBZI = "0"
    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_IMOBZI)
    imobzi_clients = [c for c in clients if c.get("imobzi_api_secret") or c.get("api_secret")]
    if not imobzi_clients:
        print("Nenhum cliente Imobzi encontrado.")
        return
    if not date_start:
    ...
    create_tables()
    for client in imobzi_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando Imobzi: {company}")
        try:
            data = extract_imobzi_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Imobzi para {company}: {e}")
