...
from connectors.moskit import MoskitConnector
...
def extract_moskit_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = MoskitConnector(
        api_key=credentials.get("api_key") or credentials.get("moskit_api_key"),
        api_url=credentials.get("api_url") or credentials.get("moskit_api_url"),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"moskit/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload Moskit MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="Moskit to ClickHouse")
def moskit_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_MOSKIT = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_MOSKIT)

    moskit_clients = [c for c in clients if c.get("moskit_api_key") or c.get("api_key")]

    if not moskit_clients:
        print("Nenhum cliente Moskit encontrado na planilha.")
        return
    ...
    for client in moskit_clients:
        company = client.get("project_id", "Unknown")
        print(f"--> Processando Moskit: {company}")

        try:
            data = extract_moskit_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Moskit para {company}: {e}")
