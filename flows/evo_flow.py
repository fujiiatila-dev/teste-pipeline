...
from connectors.evo import EvoConnector
...
def extract_evo_data(date_start: datetime, date_stop: datetime, credentials: dict):
    connector = EvoConnector(
        username=credentials.get("username") or credentials.get("evo_username"),
        password=credentials.get("password") or credentials.get("evo_password"),
        api_url=credentials.get("api_url") or credentials.get("evo_api_url"),
    )
...
def load_to_clickhouse(data_dict: dict, credentials: dict, dt_stop: datetime):
    ...
    for table_name, df in data_dict.items():
        if not df.empty:
            bucket = "raw-data"
            s3_key = f"evo/{company_name}/{table_name}_run_{date_path}.parquet"

            print(f"[{table_name}] Upload Evo MinIO ({len(df)} linhas) -> s3://{bucket}/{s3_key}")
...
@flow(name="Evo to ClickHouse")
def evo_pipeline(date_start: str = None, date_stop: str = None):
    SHEET_ID = "1ZA4rVPpHqDNvdw7t1gajgoCeV1uAaIM_sdI90BUCKIE"
    GID_EVO = "0"

    manager = GSheetsManager(sheet_id=SHEET_ID)
    clients = manager.get_tab_data(gid=GID_EVO)

    evo_clients = [c for c in clients if c.get("evo_username") or c.get("username")]

    if not evo_clients:
        print("Nenhum cliente Evo encontrado na planilha.")
        return
    ...
    for client in evo_clients:
        ...
        print(f"--> Processando Evo: {company}")

        try:
            data = extract_evo_data(dt_start, dt_stop, credentials=client)
            load_to_clickhouse(data, client, dt_stop)
        except Exception as e:
            print(f"Falha ao rodar Evo para {company}: {e}")
